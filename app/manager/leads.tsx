...
function ViewModal({ lead, onClose, theme }: ViewModalProps) {
  const statusColor = getStatusColor(lead.status);
  const confidencePct = lead.confidence_score ? `${Math.round(lead.confidence_score * 100)}%` : '—';
  const modalSubtitle = `${lead.title} · ${lead.company}`;
  const uppercaseStatus = lead.status ? lead.status.toUpperCase() : 'NEW';
  const progressLabel = getFollowupLabel(lead.followup_status);
  const aiNotesLabel = lead.ai_notes || 'Pending AI analysis.';
  const scannedLabel = lead.scanned_by_name || '—';
  const intentLabel = lead.customer_intent || '—';
  const dateLabel = new Date(lead.created_at).toLocaleString('en-SG');
        ...
        <Pressable style={[modal.sheet, { backgroundColor: theme.card }]} onPress={(e) => e.stopPropagation()}>
          ...
          <View style={modal.leadHeader}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[modal.leadSub, { color: theme.subText }]}>{modalSubtitle}</Text>
            </View>
            <View style={[modal.statusBadge, { borderColor: statusColor, backgroundColor: statusColor + '15' }]}>
              <Text style={[modal.statusBadgeText, { color: statusColor }]}>{uppercaseStatus}</Text>
            </View>
          </View>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <Text style={[modal.fieldValue, { color: theme.text }]}>{intentLabel}</Text>
            <Text style={modal.fieldLabel}>Follow-up Progress</Text>
            <Text style={[modal.fieldValue, { color: theme.text, fontWeight: '700' }]}>{progressLabel}</Text>
            ...
            <Text style={[modal.fieldValue, { color: theme.text }]}>{scannedLabel}</Text>
            <Text style={modal.fieldLabel}>AI Notes</Text>
            <Text style={[modal.fieldValue, { color: theme.text }]}>{aiNotesLabel}</Text>
            <Text style={modal.fieldLabel}>Captured At</Text>
            <Text style={[modal.fieldValue, { color: theme.text }]}>{dateLabel}</Text>
          </ScrollView>
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.bg }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.navy || '#0f172a', paddingTop: headerPaddingTop }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.headerSub}>{leadsLengthLabel}</Text>
        </View>
      </View>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item: lead }) => {
              ...
              const statusColor = getStatusColor(lead.status);
              const cardSubtitle = `${lead.title} · ${lead.company}`;
              const cardFollowupLabel = `followup: ${lead.followup_status || 'pending'}`;
              const formattedDateString = formatDate(lead.created_at);
              const itemStatusUpper = lead.status ? lead.status.toUpperCase() : 'NEW';
                ...
                <View style={[styles.card, { backgroundColor: theme.card }]}>
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={[styles.leadSub, { color: theme.subText }]} numberOfLines={1}>{cardSubtitle}</Text>
                      <Text style={[styles.leadEmail, { color: theme.subText }]} numberOfLines={1}>{lead.email}</Text>
                      <Text style={[styles.leadTime, { color: theme.subText }]}>{formattedDateString}</Text>
                    </View>
                    <View style={{ alignItems: 'flex-end', gap: 4 }}>
                      <View style={[styles.statusPill, { backgroundColor: statusColor + '18' }]}>
                        <Text style={[styles.statusPillText, { color: statusColor }]}>{itemStatusUpper}</Text>
                      </View>
