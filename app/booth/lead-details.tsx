...
export default function DashboardScreen() {
  ...
  const [dailyLeads,     setDailyLeads]     = useState<{label: string, count: number}[]>([]);
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      // Build hourly buckets: 8am–8pm in 2-hour slots
      const SLOTS = [
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      // Decode user id from JWT directly — no backend call needed
      const me = parseJwt(token);
      const userId = me?.sub || me?.id || me?.user_id;
      ...
      // Fallback: match client-side in case of type mismatch
      if (!fullUser) {
  ...
  const handleLogout = async () => {
    // Warn if offline — logging out will clear the token needed for offline access
    try {
      ...
      // NetInfo failed — proceed with normal logout
    }
    await SecureStore.deleteItemAsync('token');
    await SecureStore.deleteItemAsync('role');
    setShowProfile(false);
      ...
      <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.overviewStrip, { backgroundColor: theme.card }]}>
          <View style={styles.overviewItem}>
            <Text style={[styles.overviewNumber, { color: theme.text }]}>
              {loadingStats ? '—' : totalLeads}
            </Text>
            <Text style={[styles.overviewLabel, { color: theme.subText }]}>Total Captured</Text>
          </View>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={[styles.statCard, { backgroundColor: theme.card }]}>
            <Text style={[styles.chartTitle, { color: theme.subText }]}>ACTIVITY TODAY</Text>
            {loadingStats ? (
              <ActivityIndicator size="small" color={theme.navy} style={{ marginTop: 16 }} />
            ) : (
              <View style={styles.chartContainer}>
                {(() => {
                  const maxCount = Math.max(...dailyLeads.map(d => d.count), 1);
                  const currentHour = new Date().getHours();
                  return dailyLeads.map((slot, i) => {
                    const slotStart = [8, 10, 12, 14, 16, 18][i];
                    const isPast    = currentHour >= slotStart + 2;
                    const isCurrent = currentHour >= slotStart && currentHour < slotStart + 2;
                    const barHeight = Math.max((slot.count / maxCount) * 64, slot.count > 0 ? 6 : 2);
                    return (
                      <View key={slot.label} style={styles.chartBar}>
                        <Text style={[styles.chartCount, { color: slot.count > 0 ? theme.text : theme.subText }]}>
                        ...
                        <Text style={[styles.chartLabel, { color: isCurrent ? theme.accent : theme.subText }]}>
                          {slot.label}
                        </Text>
                        {isCurrent && <View style={[styles.chartNowDot, { backgroundColor: theme.accent }]} />}
                      </View>
                    );
                  });
                })()}
...
const styles = StyleSheet.create({
  ...
  chartTitle: { fontSize: 10, fontWeight: '700', letterSpacing: 1, marginBottom: 12 },
  chartContainer: { flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', height: 96, paddingTop: 16 },
  chartBar: { flex: 1, alignItems: 'center', gap: 4 },
  chartCount: { fontSize: 10, fontWeight: '700', minHeight: 14 },
  chartBarTrack: { width: '60%', height: 64, justifyContent: 'flex-end', alignItems: 'center' },
  chartBarFill: { width: '100%', borderRadius: 4 },
  chartLabel: { fontSize: 9, fontWeight: '600', marginTop: 2 },
  chartNowDot: { width: 4, height: 4, borderRadius: 2, marginTop: 2 },
  ...
  emptyIconRing: { width: 80, height: 80, borderRadius: 40, borderWidth: 1.5, alignItems: 'center', justifyContent: 'center', marginBottom: 4 },
  emptyTitle: { fontSize: 16, fontWeight: '700' },
  emptyText: { fontSize: 15, fontWeight: '600' },
  emptySubText: { fontSize: 13, textAlign: 'center', lineHeight: 20 },
  emptyCta: { borderRadius: 12, paddingVertical: 10, paddingHorizontal: 24, marginTop: 6 },
  emptyCtaText: { color: '#fff', fontSize: 12, fontWeight: '800', letterSpacing: 1.2 },
  ...
  profileLoading: { paddingVertical: 40, alignItems: 'center' },
  profileContent: { alignItems: 'center', paddingVertical: 16, gap: 12 },
  profileAvatarRing: { width: 72, height: 72, borderRadius: 36, backgroundColor: '#1a1a2e', alignItems: 'center', justifyContent: 'center' },
  profileAvatarText: { color: '#fff', fontSize: 28, fontWeight: '800' },
  roleBadge: { backgroundColor: '#f1f5f9', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 4 },
  roleBadgeText: { fontSize: 11, fontWeight: '700', color: '#475569', letterSpacing: 1.5 },
  profileDetails: { width: '100%', backgroundColor: '#f8fafc', borderRadius: 14, padding: 16, gap: 4, marginTop: 4 },
  profileRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 10 },
  profileRowBorder: { borderTopWidth: 1, borderTopColor: '#e2e8f0' },
  profileRowText: { flex: 1 },
  profileRowLabel: { fontSize: 11, color: '#94a3b8', fontWeight: '600', letterSpacing: 0.5 },
  profileRowValue: { fontSize: 14, color: '#0f172a', fontWeight: '600', marginTop: 2 },
  ...
  dropdown: { position: 'absolute', top: Platform.OS === 'android' ? 80 : 100, right: 16, width: 280, backgroundColor: '#fff', borderRadius: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.15, shadowRadius: 20, elevation: 12, zIndex: 101, overflow: 'visible' },
  dropdownArrow: { position: 'absolute', top: -8, right: 14, width: 16, height: 16, backgroundColor: '#fff', transform: [{ rotate: '45deg' }], shadowColor: '#000', shadowOffset: { width: -2, height: -2 }, shadowOpacity: 0.06, shadowRadius: 2 },
