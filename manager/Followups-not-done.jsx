...
export default function FollowupsNotDone() {
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.bg }]}>
      ...
      {/* HEADER */}
      <View style={[styles.header, { backgroundColor: "#dc2626", paddingTop: Platform.OS === "android" ? (StatusBar.currentHeight ?? 24) + 8 : 12 }]}>
        <TouchableOpacity onPress={() => router.back()} style={styles.backBtn}>
          <Ionicons name="arrow-back" size={22} color="#fff" />
        </TouchableOpacity>
        <Text style={styles.headerText}>Follow-ups not done</Text>
        <View style={{ width: 22 }} />
      </View>
      ...
      {!loading && !error && (
        <ScrollView
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetchLeads(true)} tintColor="#dc2626" />}
        >
          {leads.length === 0 && (
            <View style={styles.centered}>
              <Ionicons name="checkmark-circle-outline" size={48} color="#16a34a" />
              <Text style={[styles.emptyText, { color: theme.subText }]}>No low/medium priority leads.</Text>
            </View>
          )}
      ...
      {selected && (
        <View style={styles.overlay}>
          <View style={[styles.popup, { backgroundColor: theme.card }]}>
            ...
            <View style={[styles.popupDivider, { backgroundColor: theme.bg }]} />
            <TouchableOpacity onPress={() => setSelected(null)} style={[styles.closeBtn, { backgroundColor: "#dc2626" }]}>
